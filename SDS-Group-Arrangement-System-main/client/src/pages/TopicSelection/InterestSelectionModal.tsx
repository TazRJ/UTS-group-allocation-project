...
export default function InterestSelectionModal({
  ...
  const handleChoiceChange = (
    ...
    const newAvailableOptions = availableOptions.map((options, i) =>
      i === index
        ...
        : options.filter((option) => option !== event.target.value).concat(prevSelection ? [prevSelection] : []) // filter out the selected option in other dropdowns and add back the previously selected option
    );
  ...
  const handSubmit = () => {
    ...
    setError('')
    handleInputChange('interests', selections)
    closeDialog('interest')
  }
